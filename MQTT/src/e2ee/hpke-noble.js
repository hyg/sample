/**
 * HPKE 加密模块 - 基于 @panva/hpke-noble
 * 
 * 使用 RFC 9180 HPKE 标准实现
 * 套件：DHKEM-X25519-HKDF-SHA256/HKDF-SHA256/AES-128-GCM
 * 
 * 注意：@panva/hpke-noble 使用 @noble/curves，直接接受原始字节
 */
...
function kemEncrypt(recipientPublicKey, info) {
  ...
  return { enc: ephemeralPublicKey, sharedSecret: dh };
}
...
export function deriveChainKeys(rootSeed) {
  ...
  const initChainKey = hkdfSingle(new Uint8Array(), rootSeed, encoder.encode('anp-e2ee-init'), 32);
  const respChainKey = hkdfSingle(new Uint8Array(), rootSeed, encoder.encode('anp-e2ee-resp'), 32);
