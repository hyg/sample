...
| `len(inbox.e2ee_encrypted_from) > 0` | 通知仍有加密消息存在；可使用 `check_inbox.py`、`check_inbox.py --history`，或在需要时用 `e2ee_messaging.py --process --peer <DID>` 做修复处理 |
...
| 操作 | 描述 | 优先级 |
|------|------|--------|
| **查看仪表盘** | `check_status.py`——一览身份、收件箱、握手状态和待解密来源（默认已开启 E2EE 自动处理） | 🔴 首先执行 |
