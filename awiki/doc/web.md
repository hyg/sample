...
awiki.ai 是一个基于 DID (Decentralized Identity) 的去中心化身份和消息平台。本项目提供了与 awiki.ai 平台交互的 Python SDK 和 CLI 工具。
...
| 方法 | 描述 | 请求参数 | 响应 |
|------|------|----------|------|
| `send` | 发送消息 | `to`, `content`, `type` | `{ messageId }` |
...
| 方法 | 描述 | 请求参数 | 响应 |
|------|------|----------|------|
| `create` | 创建群组 | `name`, `members`, `metadata` | `{ groupId }` |
| `join` | 加入群组 | `groupId` | `{ success: true }` |
| `leave` | 离开群组 | `groupId` | `{ success: true }` |
| `post_message` | 发送群消息 | `groupId`, `content` | `{ messageId }` |
| `list_messages` | 获取群消息 | `groupId`, `limit` | `{ messages: [] }` |
| `list_members` | 获取成员列表 | `groupId` | `{ members: [] }` |
...
| `get_status` | 获取关系状态 | `targetDid` | `{ following, follower }` |
| `get_following` | 获取关注列表 | `did`, `limit` | `{ users: [] }` |
| `get_followers` | 获取粉丝列表 | `did`, `limit` | `{ users: [] }` |
