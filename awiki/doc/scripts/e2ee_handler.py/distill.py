...
# 添加 python/scripts 到路�?BASE_DIR = Path(r"D:\huangyg\git\sample\awiki\python\scripts")
...
async def test_e2ee_handler() -> None:
    ...
    # 1. 测试初始�?    print_section("1. 创建 E2eeHandler 实例")
    handler = E2eeHandler(
    ...
    print(f"输出：E2eeHandler 实例已创�?)
    print(f"is_ready: {handler.is_ready}")
    
    # 2. 测试消息类型检�?    print_section("2. 测试消息类型检�?)
    ...
    # 3. 测试协议消息处理（无客户端时�?    print_section("3. 测试协议消息处理（未初始化）")
    params = {
        "type": "e2ee_init",
    ...
    # 4. 测试消息解密（无客户端时�?    print_section("4. 测试消息解密（未初始化）")
    encrypted_params = {
        "id": "msg_001",
        "type": "e2ee_msg",
    ...
    print("输入：调�?maybe_save_state() �?force_save_state()")
...
# =============================================================================
# 附录：补充场景测�?- E2EE 状态机、WebSocket 推送、协议消息处�?# =============================================================================
...
def test_e2ee_state_machine_full_cycle(
    ...
    try:
        ...
        # 步骤 1: 初始状�?        output_data["state_transitions"].append({
            "state": "uninitialized",
        ...
        # 步骤 4: 验证状�?        output_data["final_state"] = "initialized"
...
def test_handle_websocket_push_message(
    ...
    try:
        ...
        # 步骤 2: 尝试解密（由于没有客户端，应返回错误�?        decrypt_result = asyncio.run(handler.decrypt_message(input_data["mock_websocket_message"]))
...
def test_handle_protocol_message_all_types(
    ...
    - e2ee_ack: 无响应（会话建立�?    - e2ee_rekey: 返回确认
    - e2ee_error: 记录错误
    - e2ee_msg: 尝试解密
    """
    input_data = {
...
def _record_e2ee_handler_test(
    ...
    golden_record = {
        "timestamp": Path(__file__).stat().st_mtime,  # 使用时间�?        "script": "e2ee_handler.py",
        "scenario": scenario,
        "input": input_data,
        "output": output_data,
        "success": success,
