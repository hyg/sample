...
# 添加 python/scripts 目录到路径以便导�?scripts_dir = Path(__file__).resolve().parent.parent.parent.parent / "python" / "scripts"
sys.path.insert(0, str(scripts_dir))

# 添加 python 目录到路径以便导�?utils
...
class DistillRecorder:
    ...
    def save(self):
        ...
        logger.info(f"保存蒸馏记录�?{self.output_path}")
...
def main():
    """执行蒸馏脚本�?""
    logger.info("=" * 60)
    logger.info("开始执�?local_store.py 蒸馏脚本")
    logger.info("=" * 60)
