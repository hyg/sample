...
# 需要到�?python/scripts 目录（utils 在此目录中）
...
def main() -> None:
    ...
    group = parser.add_mutually_exclusive_group(required=False)  # 改为可�?    group.add_argument("--handshake", type=str, help="预初始化 E2EE 会话")
    group.add_argument("--send", type=str, help="发送加密消息并自动初始化会�?)
    group.add_argument("--process", action="store_true", help="处理收件�?E2EE 消息")
    group.add_argument("--list-failed", action="store_true", help="列出失败的发件箱记录")
    group.add_argument("--retry", type=str, help="重试失败的记�?)
    group.add_argument("--drop", type=str, help="丢弃失败的记�?)
    ...
    parser.add_argument("--peer", type=str, help="对等�?DID�?-process 必需�?)
    parser.add_argument("--credential", type=str, default="default", help="凭证名称")

    args = parser.parse_args()
    
    # 如果指定--test 或无参数，执行测试场�?    if args.test or not any([args.handshake, args.send, args.process, args.list_failed, args.retry, args.drop]):
    ...
    if args.handshake:
        ...
        if record is None:
            parser.error(f"发件箱记�?'{args.retry}' 未找�?)
        asyncio.run(
            send_encrypted(
                record["peer_did"],
                record["plaintext"],
...
def test_e2ee_session_not_found(
    ...
    """测试 E2EE 会话不存在场景�?    
    数据准备:
    1. 删除 E2EE 会话状态文�?    2. 尝试发送加密消�?    
    预期结果:
    1. 自动重新建立会话（发�?e2ee_init�?    2. 消息成功发�?    3. 会话状态恢�?    """
    import os
    ...
    try:
        # 步骤 1: 删除会话状态（模拟会话丢失�?        config = SDKConfig()
        auth_result = create_authenticator(credential_name, config)
        if auth_result is None:
        ...
        # 备份会话状�?        backup_state = load_e2ee_state(my_did, peer_did, credential_name)
        
        # 删除会话状�?        _delete_e2ee_session(my_did, peer_did, credential_name)
        ...
        # 步骤 3: 验证会话已重新建�?        new_state = load_e2ee_state(my_did, peer_did, credential_name)
...
def test_e2ee_decrypt_unsupported_version(
    ...
    input_data = {
        ...
        "peer_did": peer_did,
        "credential_name": credential_name,
        "mock_version": "99.0",  # 不支持的高版�?        "supported_version": SUPPORTED_E2EE_VERSION,
    }
    ...
    try:
        # 步骤 1: 构造不支持版本的消�?        mock_content = {
            "version": "99.0",
            "ciphertext": "mock_ciphertext",
            "tag": "mock_tag",
        }
        
        # 步骤 2: 尝试解密（应抛出异常�?        config = SDKConfig()
        auth_result = create_authenticator(credential_name, config)
        if auth_result is None:
            output_data["error"] = f"Credential '{credential_name}' not found"
        ...
        try:
            ...
            # 验证错误码正�?            success = error_code == "unsupported_version" and retry_hint == "drop"
...
def test_e2ee_state_machine_full_cycle(
    ...
    try:
        ...
        # 步骤 2: 发送加密消�?(触发 initiating �?initialized)
        asyncio.run(send_encrypted(peer_did, plaintext, credential_name))
