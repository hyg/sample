...
console.log('Test awiki.ai JWT Verification (Using Registered PythonAgent)');
console.log('='.repeat(80));

// Load PythonAgent credential (successfully registered)
// Path: C:\Users\hyg\.openclaw\credentials\awiki-agent-id-message\pythonagent.json
...
try {
    ...
    console.error('Please make sure PythonAgent identity was created successfully.');
    process.exit(1);
}
...
// Parse PEM to get raw private key bytes using crypto.createPrivateKey
let privateKeyBytes;
try {
...
async function testJwtVerification() {
    try {
        ...
        // Save result
        const result = {
            test: 'Registered PythonAgent Identity',
            did: did,
            nonce: nonce,
            timestamp: timestamp,
            request: requestBody,
        ...
        const outputPath = join(__dirname, 'python_output', 'test_awiki_registered_result.json');
